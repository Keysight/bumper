...
data class TranslationUnit<out E, out T> (
     ...
     */
    fun typeEnv(builtins: Builtins) = typeDeclarations.typeEnv()
}
...
fun Collection<UnitDeclaration.TypeDeclaration>.typeEnv() = object : TypeEnv {
    ...
    override fun lookup(tlid: TLID): Either<TypeEnv.Missing, UnitDeclaration.TypeDeclaration> =
        this@typeEnv
            .asSequence()
            .filter { it.tlid == tlid }
            .maxBy { it.isDefinition } // true > false, so this finds the best declaration
            .toOption()
            .toEither { TypeEnv.Missing(tlid, false) }
}