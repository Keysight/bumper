...
internal class ShellTest {
    ...
    fun lexShellWsEscape() {
        ...
        assertEquals(1, result.args[0].parts.size)
        assertTrue(result.args[0].parts[0] is Val.Unquoted)

        // ast after eval
        assertEquals(listOf("x y"), result.eval())
    }


    @Test
    fun lexShellWsEscapeInQuote() {
        val result = Shell.line("\"x\\ y\"")

        // ast after parsing
        assertEquals(1, result.args.size)
        assertEquals(1, result.args[0].parts.size)
        assertTrue(result.args[0].parts[0] is Val.DoubleQuoted)
